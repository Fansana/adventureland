    ...
    if (reload_state) {
        ...
        if (reload_state == 'start') {
            reload_state = 'schedule',
                reload_timer = future_s(75 + parseInt(Math.random() * 25))
        }
...
function log_in(user_id, character_to_load, user_auth) {
    real_id = character_to_load;
    if (!game_loaded) {
...
function disconnect() {
    ...
    if (character) {
        $('title').html('Disconnected - ' + character.name)
    }
    if (socket) {
        socket = null,
            socket.disconnect()
...
function update_sprite(d) {
    ...
    if (d.stype == 'full') {
        ...
        if (d.npc && !d.moving && d.allow === true) {
            d.direction = 1
        }
        if (d.npc && !d.moving && d.allow === false) {
            d.direction = 0
        }
        if ((d.moving || g) && d.walking === null) {
            if (d.last_stop && msince(d.last_stop) < 320) {
                d.walking = d.last_walking
            } else {
                reset_ms_check(d, 'walk', 350),
                    d.walking = 1
            }
        } else {
            if (!(d.moving || g) && d.walking) {
                d.last_stop = new Date();
                d.last_walking = d.walking || d.last_walking || 1;
                d.walking = null
            }
        }
        if (d.walking && ms_check(d, 'walk', 350 - (d.speed / 2 || 0))) {
            d.walking++
        }
        ...
        if (!g && d.stunned) {
            ...
            if (d.walking) {
                ...
                if (d.last_stop && mssince(d.last_stop) < 180) {
                    c = [
                        0,
                        1,
                        2,
        ...
        if (d.charging && ms_check(d, 'clone', 80)) {
    ...
    if (d.stype == 'emote') {
        var a = (d.aspeed == 'slow' && 17) || (d.aspeed == 'slower' && 40) || 10;
        if (d.atype == 'flow') {
            if (ms_check(d, 'anim', a * 16.5)) {
                d.frame += 1
            }
...
function effects_logic(a) {
    if ((a.cursed || a.poisoned) && !a.cursed_effect) {
...
function add_character(data, d) {
    if (log_game_events) {
        console.log('add character ' + data.id)
    }
    var a = (d && manual_centering && 2) || 1;
    if (!D[data.skin]) {
        data.skin = 'tf_template'
    }
    var c = new_sprite(data.skin, 'full');
    if (a != 1) {
        c.scale = new PIXI.Point(a, a)
    }
    adopt_soft_properties(c, data);
    c.name = c.id;
    ...
    c.x = round(data.x);
    c.real_x = parseFloat(data.x);
    c.y = round(data.y);
    c.real_y = parseFloat(data.y);
    c.anchor.set(0.5, 1);
    c.type = 'character';
    c.me = d;
    c.awidth = c.width / a;
...
function create_map() {
    ...
    if (cached_map) {
        for (var z = 0; z <= M.tiles.length; z++) {
            if (z == M.tiles.length) {
                element = M['default']
            } else {
                element = M.tiles[z]
            }
            sprite_last[current_map][z] = 0;
            if (!tile_sprites[current_map][z]) {
                tile_sprites[current_map][z] = [
                ];
                if (!element) {
                    continue
                }
                ...
                var I = new PIXI.Rectangle(element[1], element[2], element[3], element[4]);
                var r = new PIXI.Texture(C[G.tilesets[element[0]]], I);
                element[5] = r;
                if (element[0] == 'water') {
                    element[5].type = 'water';
                    I = new PIXI.Rectangle(element[1] + 48, element[2], element[3], element[4]);
                    r = new PIXI.Texture(C[G.tilesets[element[0]]], I);
                    element[6] = r;
                    I = new PIXI.Rectangle(element[1] + 48 + 48, element[2], element[3], element[4]);
                    r = new PIXI.Texture(C[G.tilesets[element[0]]], I);
                    element[7] = r
                }
                if (element[0] == 'puzzle' || element[0] == 'custom_a') {
                    element[5].type = 'water';
                    I = new PIXI.Rectangle(element[1] + 16, element[2], element[3], element[4]);
                    r = new PIXI.Texture(C[G.tilesets[element[0]]], I);
                    element[6] = r;
                    I = new PIXI.Rectangle(element[1] + 16 + 16, element[2], element[3], element[4]);
                    r = new PIXI.Texture(C[G.tilesets[element[0]]], I);
                    element[7] = r
                }
            }
        ...
        for (var c = 0; c < 3; c++) {
            ...
            for (var z = 0; z < M.placements.length; z++) {
                var K = M.placements[z];
                if (K[3] === undefined) {
                    var q = M.tiles[K[0]],
                        f = q[3],
                        A = q[4];
                    if (sprite_last[current_map][K[0]] >= tile_sprites[current_map][K[0]].length) {
                        tile_sprites[current_map][K[0]][sprite_last[current_map][K[0]]] = new_map_tile(q)
                    }
                    var k = tile_sprites[current_map][K[0]][sprite_last[current_map][K[0]]++];
                    if (k.textures) {
                    ...
                    var q = M.tiles[K[0]],
                        f = q[3],
                        A = q[4];
                    if (abs(((K[3] - K[1]) / f + 1) * ((K[4] - K[2]) / A + 1)) > 3) {
                        var r = q[5];
                        if (q.length == 8) {
                            r = q[5 + c]
                        }
                        var k = new PIXI.extras.TilingSprite(r, K[3] - K[1] + f, K[4] - K[2] + A);
                        k.x = K[1] - M.min_x;
                        k.y = K[2] - M.min_y;
                        J[c].addChild(k);
                        H++
                        ...
                        F++;
                        for (var e = K[1]; e <= K[3]; e += f) {
                            for (var d = K[2]; d <= K[4]; d += A) {
                                if (sprite_last[current_map][K[0]] >= tile_sprites[current_map][K[0]].length) {
                                    tile_sprites[current_map][K[0]][sprite_last[current_map][K[0]]] = new_map_tile(q)
                                }
                                var k = tile_sprites[current_map][K[0]][sprite_last[current_map][K[0]]++];
            ...
            J[c].x = 0;
            J[c].y = 0;
            renderer.render(J[c], rtextures[c]);
        ...
        if (mode.destroy_tiles) {
            ...
            for (var z = 0; z <= M.tiles.length; z++) {
                for (var u = 0; u < tile_sprites[current_map][z].length; u++) {
                    tile_sprites[current_map][z][u].destroy()
                }
                tile_sprites[current_map][z] = null
            }
        }
    } else {
        for (var z = 0; z <= M.tiles.length; z++) {
            ...
            if (!tile_sprites[current_map][z]) {
                ...
                tile_sprites[current_map][z][sprite_last[current_map][z]] = new_map_tile(element)
            }
        }
    }
    if (M.groups) {
        for (var s = 0; s < M.groups.length; s++) {
            if (!M.groups[s].length) {
                continue
            }
            ...
            var m = 999999999,
                o = 99999999,
                B = - 999999999;
            for (var z = 0; z < M.groups[s].length; z++) {
                var K = M.groups[s][z],
                    q = M.tiles[K[0]];
                if (K[1] < o) {
                    o = K[1]
            ...
            for (var z = 0; z < M.groups[s].length; z++) {
                var K = M.groups[s][z];
                var k = new PIXI.Sprite(M.tiles[K[0]][5]);
                k.x = K[1] - o;
                k.y = K[2] - m;
                if (K[2] < m) {
                    m = K[2]
    ...
    for (var z = 0; z < npcs.length; z++) {
        ...
        console.log('NPC: ' + E.id);
        var l = add_npc(q, E.position, E.name, E.id);
        map.addChild(l);
        map_npcs.push(l);
        map_entities.push(l)
    }
    doors = map_info.doors || [];
    for (var z = 0; z < doors.length; z++) {
        var t = doors[z];
        var l = add_door(t);
        console.log('Door: ' + t);
        map.addChild(l);
